        ...
        output.push(
          <Circle
            ...
            cy={
              (height / 4) *
                3 *
                (1 - (x - Math.min(...dataset.data)) / this.calcScaler(dataset.data)) +
              paddingTop
            }
            r="4"
      ...
      output.push(
        <Polygon
          ...
          points={`${dataset.data
            .map(
              (x, i) =>
                `${paddingRight + (i * (width - paddingRight)) / dataset.data.length},${(height /
                  4) *
                  3 *
                  (1 - (x - Math.min(...dataset.data)) / this.calcScaler(dataset.data)) +
                  paddingTop}`
  ...
  renderLine = config => {
    ...
    data.map((dataset, index) => {
      const points = dataset.data.map(
        (x, i) =>
          `${paddingRight + (i * (width - paddingRight)) / dataset.data.length},${(height / 4) *
            3 *
            (1 - (x - Math.min(...dataset.data)) / this.calcScaler(dataset.data)) +
            paddingTop}`
      );
  ...
  getBezierLinePoints = (dataset, config) => {
    ...
    const y = i =>
      Math.floor(
        (height / 4) *
          3 *
          (1 - (dataset.data[i] - Math.min(...dataset.data)) / this.calcScaler(dataset.data)) +
          paddingTop
      );
        ...
        <Svg height={height} width={width}>
          <G>
            ...
            {this.renderHorizontalLabels({
              ...config,
              count:
                Math.min(...data.datasets[0].data) === Math.max(...data.datasets[0].data) ? 1 : 4,
